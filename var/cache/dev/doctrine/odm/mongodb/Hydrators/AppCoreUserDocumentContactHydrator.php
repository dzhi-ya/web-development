        ...
        if (isset($data['user']) || (! empty($this->class->fieldMappings['user']['nullable']) && array_key_exists('user', $data))) {
            ...
            if ($return !== null) {
                if ($this->class->fieldMappings['user']['storeAs'] !== ClassMetadata::REFERENCE_STORE_AS_ID && ! is_array($return)) {
                    throw HydratorException::associationTypeMismatch('App\Core\User\Document\Contact', 'user', 'array', gettype($return));
                }

                $className = $this->unitOfWork->getClassNameForAssociation($this->class->fieldMappings['user'], $return);
